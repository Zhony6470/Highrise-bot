...
class RoleManager:
    ...
    def __init__(self,roles_file): self.roles_file=roles_file; self.usernames={}; self.roles={}; self._legacy_vip_users=set(); self._load_roles()
    async def get_user_role(self,bot,user:User)->Role:
        ...
        if saved and user.id not in self.roles and user.username.casefold() in self.roles:
            self.roles[user.id]=saved; self.roles.pop(user.username.casefold(),None); self.usernames[user.id]=user.username; self._save_roles()
    ...
    async def set_role(self,bot,user_id,role:Role,username=""):
        ...
        if role=="user":
            self.roles.pop(user_id,None); self.usernames.pop(user_id,None)
        else:
            self.roles[user_id]=role
            if username:self.usernames[user_id]=username
        self._save_roles()
    ...
    def _load_roles(self):
        ...
        self.usernames={str(k):str(v) for k,v in (data.get("usernames",{}) or {}).items()}
        raw=data.get("users",{})
        if isinstance(raw,dict):
            for key,role in raw.items():
                if role in {"mod","vip","designer","user"}:self.roles[str(key)]=role
    def _save_roles(self):
        data=load_json(self.roles_file,default={}); data=data if isinstance(data,dict) else {}; data["users"]=self.roles; data["usernames"]=self.usernames; save_json(self.roles_file,data)
